...
function ProfilesToRemove({jsons,fetchProjects, checkLimiter }){
    ...
    const [toggleSelectorProcessed, setToggleProcessed] = useState(true)
   ...
   peopleToRemove()
    ...
    return(
        <section className='h-[60%] mt-10  flex '>
          ...
          <button className='mx-10 my-5 text-xl custom-bottom-border hover:border-orange-300 focus:border-orange-300 ' onClick={peopleToRemove}>Show me 10 profiles</button> 

          </div>


          <div  className='flex justify-center items-center ml-[10%] mt-5 max-w-[40%]'>
          <RenderTenRemove files={listToRemove} pID={IDprojects} fetchProjects={fetchProjects} checkLimiter={checkLimiter}/>
