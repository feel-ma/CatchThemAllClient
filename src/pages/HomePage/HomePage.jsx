...
console.log(fetchUser)
  ...
  if((fetchedUser.lastDayOnline != dayOfMonth || !fetchedUser.lastDayOnline) && fetchedUser ){
    ...
    const authToken = localStorage.getItem("authToken");
          axios
            ...
            .catch((error) => {
              console.error(error);
              setErrorMessage("Failed to create xxx.");
            });
    

  }else{console.log("ELSEEEE day")}
  ...
  }else{console.log("ELSEEEE month")}

}


  function reFetchThemAll(){
    fetchProjects()
    fetchUser()
  ...
  const fetchProjects = async () => {
    try {
     ...
     // console.log("fetchtry Jsons",response.data)
    } catch (error) {
      console.error(error);
    }
  };
 ...
 function stopHere(){
  ...
  fetchedUser.removedLimitAlert ? null : (
    <div className="mt-10 custom">
      ...
      <button className="mx-5 bg-lime-400 p-10 rounded-2xl text-xl text-white font-bold border-2 border-gray hover:border-orange-300 shadow-sm focus:border-orange-300"  onClick={showADD}>Show Profiles to Add</button>
      <button className="mx-5 bg-lime-400 p-10 rounded-2xl text-xl text-white font-bold border-2 border-gray hover:border-orange-300 shadow-sm focus:border-orange-300" onClick={showRemove}>Show Profiles to Remove</button>
      <button className="mx-5 bg-lime-400 p-10 rounded-2xl text-xl text-white font-bold border-2 border-gray hover:border-orange-300 shadow-sm focus:border-orange-300" onClick={stopHere}>Stop Here</button>
      </div>
    

      {showToAdd &&     <ProfilesToAdd jsons={jsons} setpID={setpID} pID={pID} fetchProjects={fetchProjects}  checkLimiter={checkLimiter}  />}
      {showToRemove && <ProfilesToRemove jsons={jsons} setpID={setpID} pID={pID} fetchProjects={fetchProjects} checkLimiter={checkLimiter}  /> }
...
{fetchedUser && (
  fetchedUser.removedLimitAlert ? null : (
    <div className=" mt-10">
      <div className="">
      The limiter is saying {limiter}
      </div>
      <button className=" mt-4 px-4 py-2 bg-red-400 rounded-full mx-5 text-white font-bold" onClick={checkLimiter}>CHECK</button>
    </div>
  )
)}

      {limitReched && <div>{timeHasPassed ?  <div className="text-xl mt-4">YES, you can go (reload if you see nothing)</div> : <div className="text-xl mt-16"> NO, you have to wait</div> }


      { timeHasPassed ? null : <div>

{showTimeLeft&& <div className="text-xl mt-5"> Time left: {minutesLeft} minutes and {secondsLeft} seconds</div> }

<button className="px-4 py-2 mt-4 bg-red-400 rounded-full mx-5 text-white" onClick={checkTime}> Can I go?</button>
</div>
}
...
<h2 className="mt-2">How many Actions Last Time: {fetchedUser.lastActionsCount} </h2>     
<h2 className="mt-2">How many Actions Today: {fetchedUser.actionsDay} </h2>
<button className=" mt-3 px-3 py-2 bg-red-400 rounded-full mx-5 text-white" onClick={checkDay}> CheckDay?</button>
